...
export function RequirednessBadge({ item, hasRule = false }: { item: NewDossierDocumentType; hasRule?: boolean }) {
  ...
  if (text.includes('при наличии') || text.includes('при необходимости')) {
    return <Badge className="bg-amber-100 text-amber-800 hover:bg-amber-100">При наличии</Badge>;
  }
  return <Badge className="bg-red-100 text-red-800 hover:bg-red-100">Обязателен</Badge>;
}
