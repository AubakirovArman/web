    ...
    <div className="min-w-0 space-y-4">
      <Card className="border-teal-200/70 bg-gradient-to-br from-background to-teal-50/40 dark:to-teal-950/20">
        ...
        <CardContent className="space-y-4">
          {document.status === 'pending' && <EmptyState title="Документ еще не обработан Gemma" text="Он уже включен в экспериментальный список. Запустите скрипт с большим --max-documents или --all." />}
      ...
      {intelligence && (
        <Tabs defaultValue="requirements" className="space-y-4">
          ...
          <TabsContent value="requirements"><EntityList empty="Gemma не нашла требований к документам." items={intelligence.requirements} fields={[["title", "Требование"], ["requirement_text", "Описание"], ["applies_to_document", "Документ"], ["procedure", "Процедура"], ["condition", "Условие"], ["criticality", "Критичность"], ["why_it_matters", "Почему важно"], ["source_point", "Пункт"]]} /></TabsContent>
          <TabsContent value="documents"><EntityList empty="Gemma не нашла типы документов." items={intelligence.document_types} fields={[["code", "Код"], ["name", "Тип документа"], ["mapped_guess", "Похоже на наш тип"], ["procedure", "Процедура"], ["requiredness", "Обязательность"], ["condition", "Условие"], ["why_needed", "Зачем нужен"], ["source_point", "Пункт"]]} /></TabsContent>
          <TabsContent value="parameters"><EntityList empty="Gemma не нашла параметры заявки." items={intelligence.applicant_parameters} fields={[["key", "Ключ"], ["label", "Параметр"], ["type", "Тип"], ["options", "Варианты"], ["why_needed", "Зачем нужен"], ["source_point", "Пункт"]]} /></TabsContent>
          <TabsContent value="dependencies"><EntityList empty="Gemma не нашла зависимостей." items={intelligence.dependencies} fields={[["condition_text", "Если"], ["if_parameters", "Параметры"], ["then_required_documents", "Тогда документы"], ["then_checks", "Тогда проверки"], ["explanation", "Объяснение"], ["source_point", "Пункт"]]} /></TabsContent>
          <TabsContent value="checks"><EntityList empty="Gemma не нашла проверок." items={intelligence.checks} fields={[["name", "Проверка"], ["check_type", "Тип"], ["target_document", "Документ"], ["automation_hint", "Как автоматизировать"], ["source_point", "Пункт"]]} /></TabsContent>
